...
- `Cow<T>` (Clone on Write) sahiplik ve borçlanmayı bir arada yöneten standart kütüphane enum'udur; veriyi yalnızca değiştirilmesi gerektiği anda klonlar, aksi hâlde referansını taşır. Böylece gereksiz heap tahsisatı önlenir: okuma yeterli olduğunda `Borrowed`, mutasyon gerektiğinde `Owned` kolu devreye girer (kaynak: 2025-04-03-bellek-yonetiminde-verimlilik-icin-ipuclari-rust-odakli.md).
